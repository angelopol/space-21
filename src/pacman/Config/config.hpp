...
class Config {
 public:
  ...
  const string window_title = "SPACE-21";
  ...
  const float frightened_timer = 3.0f * window_framerate / tile_size;
  const float chase_timer = 35.0f * window_framerate / tile_size;
  const float scatter_timer = 6.0f * window_framerate / tile_size;
  const float hurt_timer = 3.00f * window_framerate / tile_size;
